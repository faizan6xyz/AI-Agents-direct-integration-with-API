...
def insert_time(user_id, container_id, scheduled_time, access_token):    # time should be give in the isoformat iniitally as argument 
    conn = get_conn()
    conn.execute(
        "INSERT INTO schedule (user_id, container_id, time, access_token) VALUES (?, ?, ?, ?)",
        (user_id, container_id, scheduled_time, access_token)
    )
    conn.commit()
    conn.close()
...
def get_containers_due(now):
    conn = get_conn()
    cur = conn.execute(
        "SELECT id, container_id, access_token, user_id FROM schedule WHERE time < ?", (now,)
    )
    rows = cur.fetchall()
    conn.close()
    return rows
