...
def create_order():
    try:
        ...
        order_payload = {"intent": "CAPTURE",
            "purchase_units": [{
                "amount": {
                    "currency_code": currency,
                    "value": str(amount_float)},
                "description": description}]} 
        response = requests.post(f"{PAYPAL_API_BASE}/v2/checkout/orders",
            headers={'Authorization': f'Bearer {access_token}',
                'Content-Type': 'application/json'},json=order_payload)
        if response.status_code in [200, 201]:
            ...
            return jsonify({'success': False,'error': f"Failed to create order: {response.text}"}), 500
    except Exception as e:
...
def capture_payment():
    try:
        ...
        access_token = get_paypal_access_token()
        response = requests.post(f"{PAYPAL_API_BASE}/v2/checkout/orders/{order_id}/capture",
        ...
        if response.status_code in [200, 201]:
            ...
            return jsonify({'success': False,
                'error': f"Failed to capture payment: {response.text}"}), 500
    except Exception as e:
        return jsonify({'success': False,'error': str(e)}), 500
...
def check_order_status(order_id):
    try:
        access_token = get_paypal_access_token()
        response = requests.get(f"{PAYPAL_API_BASE}/v2/checkout/orders/{order_id}",
            headers={'Authorization': f'Bearer {access_token}','Content-Type': 'application/json'})
        if response.status_code == 200:
            order_data = response.json()
            return jsonify({'success': True,
