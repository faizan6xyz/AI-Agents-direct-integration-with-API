...
def compress_image(input_path, output_path=None, target_bytes=None,
                    max_dimension=MAX_IMAGE_DIMENSION_DEFAULT):
    ...
    if target_bytes and final_size > target_bytes:
        logger.warning(
            f"Could not reach target size for '{input_path}': "
...
def compress_video(input_path, output_path=None, target_bytes=None,
                    crf=28, max_height=720):
    _require_ffmpeg()
    resolved_in, resolved_out = _resolve_paths(input_path, output_path, "_compressed")
...
def compress_for_platform(input_path, media_type, platform="gmail", output_path=None):
    limits = {
        "gmail": {"image": 25 * 1024 * 1024, "audio": 25 * 1024 * 1024, "video": 25 * 1024 * 1024},
...
if __name__ == "__main__":
    ...
    print(f"Saved: {result} ({os.path.getsize(result) / 1024:.0f} KB)")